...
class TrialBalanceAnalyticReport(models.AbstractModel):
    ...
    def _get_trial_balance(self, accounts_data, total_amount, show_hierarchy):
        if show_hierarchy:
            ...
            # Ensure deterministic ordering: prefer analytic account code
            # (accounts_data['code'] is already account.code or name fallback)
            trial_balance = sorted(
                trial_balance, key=lambda k: (k.get("code") or "").lower()
            )
